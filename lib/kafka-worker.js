...
module.exports = function(config, onMessageCallback) {
  ...
  var messageTracking = config.messageTracking !== false;
  ...
  let consumerGroup = consumerConfig.options.groupId;
  let workerInfo = [
    `consumerGroup: ${consumerGroup}`,
    `topic: ${consumerConfig.topic}`,
    `messageTracking: ${messageTracking}`
  ].join(' - ');
  logger.info(`[STARTED] ${workerInfo}`);
  ...
  this.close = function(callback) {
    logger.info(`[CLOSING] ${workerInfo}`);
    consumer.close(process.env.FORCE_COMMIT === 'true', function() {
      logger.info(`[CLOSED] ${workerInfo}`);
