...
module.exports = function(config, onMessageCallback) {
  ...
  consumer.on('message', function(message) {
    ...
    try {
      ...
      log('error', 'onMessage', message, ['Error: ' + e.message || e]);
  ...
  this.sendMessages = function(topic, messages, callback) {
    ...
    Q.all(tasks)
        .then(function(nextMessageIdsList) {
          self.track(topic, messages, messageTraceConfig.states.sent);
  ...
  this.retryAndCommit = function(message) {
    var logStrings = ['Retry topic: ' + retryTopic];
    log('warn', 'retryAndCommit', message, logStrings);
    ...
    this.sendMessages(retryTopic, [message.value])
        ...
        .catch(function(err) {
          logStrings.push('Error message: ' + err.message || err);
          log('error', 'retryAndCommit', message, logStrings);
          setTimeout(function() {
            self.retryAndCommit(message);
          }, 5000);
        });
  ...
  this.track = function(topic, messages, status, nextMessageIds, callback) {
    ...
    producer.send(payloads, function(err, data) {
      if (err) {
        logStrings = _.union(logStrings, ['SEND FAILED', 'Error message: ' + err]);
