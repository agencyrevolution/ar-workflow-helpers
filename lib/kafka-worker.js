...
var loggerHelpers = require('./logger');
...
module.exports = function(config, logger, onMessageCallback) {
  ...
  var log = function(logLevel, funcName, message, logMessages) {
    loggerHelpers.logWorker(logger, logLevel, consumerConfig.groupId, funcName, message, logMessages);
  };
  ...
  this.commitMessage = function(message, callback) {
    ...
    consumer.commit(true, function(err, data) {
      ...
      if (areMessagesProceeded()) {
        logger.verbose('All fetched messages have been committed! Resume consumer to send next fetch request...', '\n');
        consumer.resume();
      }

      log('verbose', 'commitMessage(message)', message, ['MESSAGE COMMITED']);
  ...
  this.commitOffset = function(message, callback) {
    ...
    offset.commit(groupId, payloads, function(err, data) {
      if (err) {
        ...
        log('verbose', 'commitOffset', message, ['groupId: ' + groupId, 'OFFSET COMMITTED!']);
        deferred.resolve(data);
      }
    });
  ...
  this.sendMessages = function(topic, messages, callback) {
    ...
    var log = function(logLevel, logStrings) {
      logStrings = _.union([
        'kafkaWorker - sendMessages(payloads)',
        'topic: ' + topic,
  ...
  this.track = function(topic, messages, status, nextMessageIds, callback) {
    logger.verbose('kafkaWorker - track(message)',
        '\n\t message topic: ', topic,
        '\n\t status: ', status, '\n');
    ...
    producer.send(payloads, function(err, data) {
      ...
      logger.verbose('kafkaWorker - track() - MESSAGE SENT: ',
          '\n\t message topic: ', payloads.map(function(p) {
            return p.topic;
          }));
  ...
  this.handleOffsetOutOfRange = function(topic, partition, callback) {
    logger.verbose('kafkaWorker - handleOffsetOutOfRange()',
        '\n\t topic: ', topic,
        '\n\t partition: ', partition, '\n');
    var deferred = Q.defer();
