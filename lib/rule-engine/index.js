...
var RuleEngine = function() {
  ...
  var isMatch = function(item, fields, operator, value) {
    ...
    try {
      ...
      switch (operator) {
        ...
        case operators.isNotEmpty.name:
        case operators.containAny.name:
        case operators.notContainAll.name:
      ...
      logger.log('error', 'RuleEngine', 'isMatch', logsStrings);
    }
  ...
  /**
   * Find matches by segments
   * @param  {array} segments  Segments
   * @param  {string} matchType any | all
   * @param  {array} array     Array
   * @return {array}
   */
  var findMatchesBySegments = function(segments, matchType, array) {
    let matchesList = _.map(segments, function(segment) {
      return findMatches(segment, array);
    });
    return filterByMatchType(matchesList, matchType);
  };
  ...
  var findMatches = function(rule, array) {
    ...
    if (segments && segments.length) {
      return findMatchesBySegments(segments, rule.matchType, array);
    }
    ...
    var matchesList = findMatchesListByConditions(conditions, array);
