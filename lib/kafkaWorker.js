...
module.exports = function(config, logger, onMessageCallback) {
  ...
  var log = function(logLevel, funcSignature, message, logMessages) {
    var logStrings = _.union([
      ...
      [
        ...
        'size: ' + JSON.stringify(message.value).length + ' bytes'
      ].join(' - ')
    ], logMessages);
  ...
  var logFetchData = function() {
    ...
    for (var topic in fetchData) {
      logStrings.push('topic ' + topic + ': ' + fetchData[topic].length);
    }
    logStrings.push('---------------------------------------------------------');
  ...
  consumer.on('message', function(message) {
    ...
    try {
      ...
      fetchData[message.topic] = _.union(fetchData[message.topic], [
        _.pick(message, ['partition', 'offset'])
      ]);
      logFetchData();
      ...
      log('error', 'consumer.on(message)', message);
  ...
  this.commitMessage = function(message, callback) {
    ...
    consumer.commit(true, function(err, data) {
      ...
      fetchData[message.topic] = _.reject(fetchData[message.topic], _.pick(message, ['partition', 'offset']));
