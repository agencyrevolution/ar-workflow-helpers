...
module.exports = function(config, logger, onMessageCallback) {
   ...
   * Done processing
   */
  this.done = function(message, messageValue, nextMessageIds) {
    this.commitMessage(message, messageValue, function(err, res) {
      if (err) {
        logger.error('Retry committing after 5s...');
        setTimeout(function() {
          self.done(message, messageValue);
        }, 5000);
      } else {
        self.track(message.topic, [messageValue], messageTraceConfig.states.succeeded, nextMessageIds);
      }
    });
  };
   ...
   * Hanlde retry
   */
  this.retry = function(messageValue, callback) {
